...
declare global{
    namespace PlaywrightTest {
        interface Matchers<R, T> {
            ...
            shouldMatchSchema(directory: string, fileName: string, createSchemaFlag?: boolean): Promise<R>;
        }
    }
}
export const expect = baseExpect.extend({
     async shouldMatchSchema(received: any, directory: string, fileName: string, createSchemaFlag: boolean = false) {
        let pass: boolean;
        let message: string = ''
         try {
      await validateSchema(directory, fileName, received, createSchemaFlag);
      pass = true;
